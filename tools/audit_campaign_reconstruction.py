...
CAMPAIGNS = (
    ROOT / "calibration/system_recalibration_001",
    ROOT / "characterization/system_characterization_001",
)
INDEX_KEYS = {
...
def main() -> int:
    ...
    for campaign in CAMPAIGNS:
        for path in (campaign / "readbacks").rglob("*"):
            if not path.is_file():
                continue
            if path.name in {
    ...
    manifests = []
    for campaign in CAMPAIGNS:
        for path in (campaign / "readbacks").rglob("phase_manifest.json"):
