...
- T660 recipe: `recipes/mircat_detector_alignment_2mhz.yaml` only when `Use T660 Timing` is checked
...
10. If `Use T660 Timing` is checked, apply
   `recipes/mircat_detector_alignment_2mhz.yaml` to start continuous T660-2
   timing:
...
1. Apply `recipes/safe_idle.yaml`.
...
`tests/hardware_checks/check_mircat_detector_alignment.py` uses the same workflow
