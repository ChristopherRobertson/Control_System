...
The Qt panel is implemented in `control_app/ui/widgets/experiment_builder_widget.py`
...
- Executable workflows currently live in `control_app/workflows`: MIRcat status,
...
- `recipes/ui_workflows.yaml`, `selectable_workflows.py`, and
...
- `*_widget_commands.py` and `ui_command_router.py` are the device command
  ...
  `control_app/devices/*_service.py` and should be wrapped by capability
...
is defined in `docs/default_wiring_state.md` and is not a recurring operator
