"""Apply and verify final T660-only safe idle for blocked MC-01 closeout."""
...
def main() -> int:
    status_path = HERE / "final_safe_idle_status.json"
    record = {
        "campaign_id": "system_recalibration_001",
        "phase_id": "MC-01",
        "phase_run_id": "system_recalibration_001_MC-01_001",
        "started_utc": datetime.now(UTC).isoformat(),
        "purpose": "final T660-only safe idle for blocked MC-01 closeout",
    ...
    log = (HERE / "final_safe_idle_command_log.txt").open("a", encoding="utf-8")
    manager = TimingRecipeManager(inventory=inventory, command_log=log)
    try:
        readback = manager.apply_recipe(
            SAFE_IDLE,
            output_path=HERE / "final_safe_idle_readback.json",
        )
