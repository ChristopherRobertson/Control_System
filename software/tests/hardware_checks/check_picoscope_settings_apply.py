...
Usage:
    python software/tests/hardware_checks/check_picoscope_settings_apply.py --operator "Name" --confirm-real-hardware
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--recipe",
        default="instrument/recipes/picoscope_settings_test.yaml",
...
def main() -> int:
    args = parse_args()
    run_dir = REPO_ROOT / "evidence" / "experiments" / "runs" / f"{today_stamp()}_picoscope_settings_test"
...
def _next_actions(errors: list[str]) -> list[str]:
    ...
    return [
        ...
        "Adjust instrument/recipes/picoscope_settings_test.yaml only if the test settings should differ.",
