...
def test_requested_example_distinguishes_baseline_and_zero_delay_pump():
    ...
    assert plan.phases_per_repetition == 1601
    assert plan.total_scans == 1602
    assert plan.total_pump_events == 1601
    assert plan.first_phase_delay_us == -3000
    ...
    assert plan.pump_rate_hz == 4
    baseline, first, last = (plan.event_at(i) for i in (0, 1, 1601))
    assert not baseline.pump_enabled
    assert baseline.phase_delay_us is None
    assert first.pump_enabled and first.phase_delay_us == -3000
    assert last.pump_enabled and last.phase_delay_us == 5000
    assert plan.event_at(601).pump_enabled and plan.event_at(601).phase_delay_us == 0
...
def test_exploratory_current_and_fast_hf2li_preset_leave_campaign_candidate_unchanged():
    ...
    assert all(item["rate_sps"] == pytest.approx(20_000) for item in detectors.values())
...
def test_repetitions_repeat_entire_set_instead_of_repeating_each_phase():
    ...
    assert plan.total_scans == 4806
    assert plan.total_pump_events == 4803
    for rep in range(3):
        start = rep * 1602
        assert plan.event_at(start).repetition == rep + 1
        assert not plan.event_at(start).pump_enabled
        assert plan.event_at(start + 1).phase_delay_us == -3000
        assert plan.event_at(start + 1601).phase_delay_us == 5000
...
def test_signed_grid_rounds_outward_without_float_rounding(increment, expected_count, last_delay):
    ...
    assert plan.first_phase_delay_us <= -3_000
...
def test_cadence_duration_does_not_add_rest_after_every_scan_or_a_trailing_rest():
    plan = build_phase_scan_plan(PhaseScanSettings())
    # 1,601 pumped phases plus baseline; final sweep ends 6 ms after its pump.
    assert plan.nominal_duration_s == pytest.approx(400.256)
...
def test_widget_updates_plan_and_clears_stale_preview_on_invalid_input(qt_app):
    ...
    assert widget.plan.total_scans == 1602
    assert "1,602" in widget.summary_values["total"].text()
    assert widget.phase_table.item(0, 1).text() == "Baseline · pump OFF"
    assert widget.phase_table.item(1, 2).text() == "-3,000 µs"
    widget.inputs["repetitions"].setValue(2)
    assert widget.plan.total_scans == 3204
    assert "3,204" in widget.summary_values["total"].text()
...
def test_save_plan_records_corrected_probe_fields_and_no_hardware_authorization(qt_app, monkeypatch, tmp_path):
    ...
    assert payload["derived"]["total_scans"] == 1602
    assert payload["status"] == "PLANNING_ONLY"
