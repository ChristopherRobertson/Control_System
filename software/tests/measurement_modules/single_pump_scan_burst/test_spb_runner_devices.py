...
def baselines(ctx, p):
    ...
    if p.settings.mode == "single":
        ...
        assert result["blank"]["total_scans"] == p.settings.preliminary_scan_count
    return result


@pytest.mark.parametrize("mode", ["single", "dual"])
def test_injected_full_workflow_has_exactly_one_electrical_pump_across_all_blocks(tmp_path, mode):
    p, ctx = plan(mode), context(tmp_path, mode)
    baseline = baselines(ctx, p)
    runner, adapter = build(ctx, p)
    result = runner.run(baseline=baseline)
    assert result.status == "complete", result.to_dict()
    assert adapter.pump_count == 1
    assert result.epoch["electrically_observed"]
...
def test_ambiguous_epoch_after_emission_is_retained_and_never_repeated(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="cannot be repeated"):
        runner.run(baseline=baseline)
    resumed, hardware = build(ctx, p)
    refusal = resumed.run(baseline=baseline,
...
def test_long_wait_abort_restores_preserves_epoch_and_does_not_repeat_pump(tmp_path):
    ...
    assert load_run(result.output_path)["checkpoint"]["state"]["epoch"]["electrically_observed"]
...
def test_thermal_metadata_does_not_gate_the_explicit_pump(tmp_path):
    p, ctx = plan(), context(tmp_path)
    baseline = baselines(ctx, p)
...
def test_missing_reference_retains_raw_and_unavailable_normalization_without_retry(tmp_path):
    p, ctx = plan(), context(tmp_path)
    baseline = baselines(ctx, p)
    runner, adapter = build(ctx, p)
    ...
    outcome = runner.run(baseline=baseline)
...
def test_empty_preliminary_detector_retains_exact_native_values(tmp_path):
    p, ctx = plan(), context(tmp_path)
    runner, adapter = build(ctx, p)
    capture = adapter.capture_block
    ...
    assert np.isnan(outcome["native"]["sample"]).all()
...
def test_native_reconstruction_keeps_uint64_markers_pointwise_times_and_missing_edges():
    ...
    # Negative Q-switch timing uses its observed leading falling edge, not the
    # trailing positive edge one pulse width later.
    streams[2]["dio"] = words ^ np.uint32(1 << 17)
    negative = reconstruct_native(streams, clockbase_hz=210_000_000,
        markers_cm1=[1900, 1910, 1920, 1930], expected_scans=1, pump_active_high=False)
    np.testing.assert_array_equal(negative["native_pump_sync_ticks"], ticks[[1]])
    assert negative["pump_electrical_edge"] == "falling"
...
def test_verified_continuation_preserves_epoch_and_does_not_rearm_early_pump(tmp_path):
    ...
    interrupted = first.run(baseline=baseline)
    ...
    result = second.run(baseline=baseline, continuation=retained)
    assert result.status == "complete", result.to_dict()
