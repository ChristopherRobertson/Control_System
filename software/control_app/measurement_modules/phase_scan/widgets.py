...
try:
    ...
    from PySide6.QtWidgets import (
        ...
        QGridLayout, QHBoxLayout, QHeaderView, QLabel, QLineEdit, QMessageBox, QPushButton, QScrollArea,
...
class PhaseScanWidget(QWidget):
    ...
    def __init__(self, parent=None, *, runner=None, diagnostic=None, before_start=None, preferences=None, dual_detector=False,
        ...
        from control_app.measurement_host.parameter_feedback import live_parameter_updates
        live_parameter_updates(self, self._refresh_plan)
        self._refresh_plan()
    ...
    def _build(self):
        ...
        run_label = QLineEdit(defaults.run_label)
        run_label.setObjectName("run_label")
        run_label.setPlaceholderText("Optional label")
        run_label.textChanged.connect(self._refresh_plan)
        self.inputs["run_label"] = run_label
        run_form.addRow("Run Label", run_label)
        control_layout.addWidget(run_group)
        groups = (
            ("Nd:YAG + OPO Settings", (
                ("pump_repetition_rate_hz", "Repetition Rate", " Hz", .000001, 10., 6, 1.),
                ("fire_to_qswitch_us", "FIRE - Q-SWITCH Delay", " µs", 1., 1e6, 3, 1.),
                ("pump_wavelength_nm", "Wavelength", " nm", 1., 10000., 3, 1.),
            )),
            ("MIRcat Settings", (
                ("start_wavenumber_cm1", "Start", " cm⁻¹", 1639., 2077., 3, 1.),
                ("stop_wavenumber_cm1", "Stop", " cm⁻¹", 1639., 2077., 3, 1.),
                ("scan_speed_cm1_s", "Scan Speed", " cm⁻¹/s", 1., 10000., 3, 100.),
                ("probe_repetition_rate_hz", "Repetition Rate", " Hz", 1., 10e6, 3, 1000.),
                ("probe_pulse_width_ns", "Pulse Width", " ns", 21., 1005., 3, 1.),
                ("qcl_current_ma", "Current", " mA", 250., 1000., 3, 10.),
            )),
            ("Phase Settings", (
                ("pre_pump_ms", "Pre-pump reconstruction", " ms", 0., 1e6, 3, .1),
                ("post_pump_ms", "Post-pump reconstruction", " ms", .001, 1e6, 3, .1),
                ("phase_delay_us", "Phase-Delay increment", " µs", 1., 1000., 3, 1.),
            )),
        )
        ...
        advanced = QGroupBox("HF2LI Settings")
        ...
        workflow_controls = (self.test_button, self.start_button, self.abort_button)
        ...
        self.required_settings = QLabel()
        self.required_settings.setWordWrap(True)
        summary_form.addRow(self.required_settings)
        for key, label in (("duration", "Sweep duration"), ("window", "Reconstruction window"), ("delays", "Required sweep-start range"), ("total", "Sequence scan count"), ("pump", "Nd:YAG settings"), ("elapsed", "Estimated completion"), ("probe", "MIRcat settings"), ("capacity", "Preflight capacity"), ("hf2", "Selected HF2LI"), ("resolution", "Effective resolution")):
            value = QLabel()
        ...
        self.preliminary_axis_inputs = {name: QLineEdit() for name in ("xmin", "xmax", "ymin", "ymax")}
        for name, label in (("xmin", "X min"), ("xmax", "X max"), ("ymin", "Y min"), ("ymax", "Y max")):
            editor = self.preliminary_axis_inputs[name]
            editor.setObjectName("preliminary_plot_" + name)
            editor.setMaximumWidth(105)
            editor.setPlaceholderText("Auto")
            editor.editingFinished.connect(self._apply_preliminary_axis_limits)
        auto_limits = QPushButton("Auto limits")
        auto_limits.clicked.connect(self._restore_preliminary_axis_limits)
        auto_row = QHBoxLayout()
        auto_row.addWidget(auto_limits)
        auto_row.addStretch(1)
        review_layout.addLayout(auto_row)
        plot_frame = QWidget()
    ...
    def _populate_override_choices(self):
        ...
        for key, combo in self.override_inputs.items():
            ...
            combo.setEnabled(not self.command_running() or self._current_kind == "capabilities")
    ...
    def _refresh_plan(self, *_):
        ...
        from control_app.measurement_host.parameter_feedback import required_parameter_prompts, plan_parameter_prompts
        self.required_settings.clear()
        self.required_settings.hide()
        try:
            missing = required_parameter_prompts(self)
            if missing:
                raise ValueError("\n".join(missing))
            self.plan = self.runner.configuration_preview(self.settings(), overrides=self._overrides)
        except (ValueError, RuntimeError) as exc:
            self.plan = None
            self.required_settings.setText(plan_parameter_prompts((str(exc),)))
            self.required_settings.show()
            ...
            from control_app.measurement_host.uniform_layout import effective_hf2_values
            effective_hf2_values(self, p)
            ...
            self.summary_values["pump"].setText(f"{p.settings.pump_repetition_rate_hz:g} Hz · {p.frame_period_s*1000:.9g} ms cadence · FIRE→Q-switch {p.settings.fire_to_qswitch_us:g} µs · 540 nm")
            self.summary_values["elapsed"].setText(f"At least {p.nominal_duration_s:.3f} s per sequence; setup, settling and retrieval add time")
            self.summary_values["probe"].setText(f"{p.settings.probe_repetition_rate_hz/1e6:g} MHz external probe triggering, {p.settings.probe_pulse_width_ns:g} ns TTL; MIRcat internal {p.settings.mircat_internal_repetition_rate_hz/1e6:g} MHz / {p.settings.mircat_internal_pulse_width_ns:g} ns; {p.settings.qcl_current_ma:g} mA; CH1 SIG IN +")
            capacity = p.capacity
            ...
            if self.dual_detector:
                ...
                self.summary_values["probe"].setText(f"{p.settings.probe_repetition_rate_hz/1e6:g} MHz external probe triggering, {p.settings.probe_pulse_width_ns:g} ns TTL; MIRcat internal {p.settings.mircat_internal_repetition_rate_hz/1e6:g} MHz / {p.settings.mircat_internal_pulse_width_ns:g} ns; {p.settings.qcl_current_ma:g} mA; sample + reference")
    ...
    def _update_buttons(self):
        ...
        session = bool(getattr(self.runner, "experiment_session_active", False))
        ...
        self.background_button.setEnabled(ready and not self.dual_detector and not session)
        self.load_background_button.setEnabled(valid and not busy and not self.dual_detector and not session)
        ...
        self.start_button.setEnabled(ready and preliminary)
        self.abort_button.setText("Abort Acquisition")
        self.abort_button.setEnabled((session and not busy) or (busy and self._current_kind not in {"capabilities", "end_experiment"}))
        self.save_button.setEnabled(valid and not busy)
        self.load_plan_button.setEnabled(not busy and not session)
        self.new_run_button.setEnabled(not busy)
        self.reconstruction.load_button.setEnabled(not busy)
        self.advanced_group.setEnabled(not acquiring and not session)
        self._populate_override_choices()
        for control in self.inputs.values():
            control.setEnabled(not acquiring and not session)
        if acquiring:
            ...
            text = "Device verification is pending or unsuccessful. You can edit saved choices; use Instruments → Recheck supported device choices to retry."
            ...
            text = "Compatible unpumped sample ready. Start Acquisition when ready."
        if session and not busy:
            text += " Experiment session retained: reference running, settings frozen, emission off."
    ...
    def _begin(self, kind):
        if self.command_running() or (kind not in {"capabilities", "end_experiment"} and self.plan is None):
            return
        blocker = self.before_start() if kind != "end_experiment" else None
        if blocker:
            self.scan_status.setText(str(blocker))
            return
        if kind == "capabilities":
            self._capability_check_attempted = True
        if kind not in {"capabilities", "end_experiment"}:
            descriptions = {
                ...
                "run": f"Start {self.plan.total_scans:,} sample scans including {self.plan.total_pump_events:,} pumped phases at {self.plan.settings.pump_repetition_rate_hz:g} Hz. This enables the pump FIRE and Q-switch outputs with {self.plan.settings.fire_to_qswitch_us:g} µs separation. Confirm the sample is loaded and the instrument is ready.",
            }
            if self.dual_detector:
                ...
                descriptions["run"] = (f"Start {self.plan.total_scans:,} simultaneous sample/reference scans, including {self.plan.total_pump_events:,} pumped phases at {self.plan.settings.pump_repetition_rate_hz:g} Hz. FIRE→Q-switch separation is {self.plan.settings.fire_to_qswitch_us:g} µs. Confirm the sample and reference blank are loaded and the instrument is ready.")
        ...
        save_root = None if kind == "end_experiment" else Path(self.save_root_provider()).expanduser().resolve()
        selected = {}
        if kind not in {"capabilities", "end_experiment"}:
            ...
            if callable(freeze_selection):
                try:
                    ...
                    return
        if kind == "end_experiment":
            def operation(worker):
                worker.message.emit("Ending experiment; restoring instrument settings and closing connections…")
                self.runner.end_experiment()
                return {"kind": "end_experiment"}
    ...
    def set_latest_scan(self, wavenumbers_cm1, absorption, scan_label="Preliminary unpumped sample"):
        ...
        self._restore_preliminary_axis_limits()
        if self.dual_detector:
    ...
    def _sync_preliminary_axis_inputs(self):
        values = self.canvas.axis_limits()
        for editor, value in zip(self.preliminary_axis_inputs.values(), values):
            editor.setText(f"{value:.12g}")

    def _apply_preliminary_axis_limits(self):
        try:
            values = tuple(float(self.preliminary_axis_inputs[name].text())
                           for name in ("xmin", "xmax", "ymin", "ymax"))
            if not all(math.isfinite(value) for value in values) or values[0] >= values[1] or values[2] >= values[3]:
                raise ValueError
        except ValueError:
            self._sync_preliminary_axis_inputs()
            return
        self.canvas.display_limits = values
        self.canvas.update()

    def _restore_preliminary_axis_limits(self):
    ...
    def _worker_finished(self):
        ...
        if result is not None:
            if result["kind"] == "end_experiment":
                self._new_run()
                self.scan_status.setText("Experiment ended; settings restored and connections closed.")
            elif result["kind"] == "capabilities":
    ...
    def _abort(self):
        if not self.command_running() and getattr(self.runner, "experiment_session_active", False):
            self._begin("end_experiment")
            return
        self.runner.abort()
    ...
    def instrument_state_changed(self, change):
        if self.command_running():
            ...
            return
        if getattr(self.runner, "experiment_session_active", False):
            self._pending_instrument_changes.append(change)
            self._begin("end_experiment")
            return
    ...
    def load_plan(self, path):
        if getattr(self.runner, "experiment_session_active", False):
            raise RuntimeError("End the current experiment before loading another plan")
        if self.command_running():
        ...
        for key, control in self.inputs.items():
            ...
            if isinstance(control, QLineEdit):
                if not isinstance(value, str):
                    raise ValueError(f"Saved {key} must be text")
            elif not isinstance(value, (int, float)) or not math.isfinite(value) or not control.minimum() <= value <= control.maximum():
        ...
        try:
            for key, control in self.inputs.items():
                value = getattr(settings, key)
                control.setText(value) if isinstance(control, QLineEdit) else control.setValue(value)
    ...
    def _restore_preferences(self):
        ...
        try:
            ...
            for key, value in payload.get("inputs", {}).items():
                ...
                if isinstance(control, QLineEdit) and isinstance(value, str):
                    control.setText(value)
                elif isinstance(value, (int, float)) and control.minimum() <= value <= control.maximum():
    ...
    def _save_preferences(self):
        if self.preferences is not None:
            payload = {"inputs": {key: control.text() if isinstance(control, QLineEdit) else control.value()
                                  for key, control in self.inputs.items()}, "overrides": self._overrides}
            if self.dual_detector:
