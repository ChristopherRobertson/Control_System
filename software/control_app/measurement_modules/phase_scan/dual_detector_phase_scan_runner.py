    ...
    def execute(self, kind, root, plan, *, on_scan=lambda *args: None,
        ...
        try:
            ...
            if kind == "test":
                ...
                baseline = None
            elif baseline_conflicts(plan):
                raise ValueError("Acquire a compatible preliminary unpumped sample/reference spectrum before starting the pump")
        ...
        try:
            if store is not None:
                ...
                retained_baseline = None if baseline is None else {
                    ...
                    "reviewed": bool(selected.preliminary_reviewed), "channel_balance": None if baseline.get("channel_balance") is None else baseline["channel_balance"].to_dict()}
