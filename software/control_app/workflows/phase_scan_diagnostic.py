...
def capture_inhibited_diagnostic(root: Path, *, cancel: Event | None = None,
    ...
    T660-1 C -> T660-2 TRIG IN (10 ms high); T660-2 C -> inhibited MIRcat
        ...
        try:
            ...
            for name in ("t660_1", "t660_2"):
                ...
                channels["C"] = {"enabled": True, "delay": "0ns", "width": "10ms",
                                 "polarity": "negative" if name == "t660_2" else "positive",
                                 "termination": "50OHM"}
                recipe = {"stop_first": True, "trigger_source": "OFF", "gate_mode": 0,
                          "burst_enabled": False, "predivider": 1, "channels": channels}
                if name == "t660_2":
            ...
            for index in range(count):
                ...
                store.save_scan(event, {"kind": "INHIBITED_DIAGNOSTIC", "optical_valid": False,
                    ...
                    "diagnostic_dio_bit": 21,
                    "diagnostic_dio_signal": "MIRcat DB9 pin 2 Sweep Active (observational input)",
                    "diagnostic_dio_expectation": "No sweep is commanded while inhibited; no Sweep Active transition is expected or generated by the diagnostic.",
                ...
                if unit.name == "t660_2":
                    for command in ("TFRame:STOp", *(f"TRAin:{stage}:CouNT 0" for stage in ("ACTive", "NEXT", "QUEue"))):
                        try:
                            ...
                            cleanup_errors.append(f"{unit.name} {command}: {exc}")
                try:
