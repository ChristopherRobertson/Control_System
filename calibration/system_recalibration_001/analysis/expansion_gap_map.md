...
| Visible/near-IR wavelength working reference | Coherent manufacturer documents and registered disconnected-device fields | WM-01 qualifies installed identity power/cable/adapter communications settings response states 540 nm repeatability uncertainty and validity; `[VALUE_REQUIRED]` connection fields block entry | PB-02 and every quantitative OPO-540 phase import native wavelength/status evidence and the WM-01 bundle; center wavelength is not a spectral-power fraction and 355 nm is outside the meter range. |
| Electronic OPO iris halo rejection optical attenuation splitter ratio and sample-plane transfer | OM-01 bounded pre-iris mixed-output observation plus permanent ELL15 identity and service records | ATT-01 imports WM-01; qualifies USB/API command/readback and faults; selects and locks the permanent far-field mount; optimizes the 540 nm diameter without core clipping; controls 950 nm home-sensor leakage; and measures every used transfer element and both installed splitter ports | PB-02 and OG/OV use only the qualified post-iris OPO-540 configuration; apply identified transfer corrections without assuming 50/50; another OPO wavelength requires separate iris/centroid qualification. |
...
| Pump/probe beam output and geometry | No completed quantitative evidence; OM-01 records only binary containment of the unfiltered OPO footprint | Metrology resources and permanent OPO-540 iris configuration only in OM-01/ATT-01 | PB-01 through OV-01 measure the final path; OPO-540 results always retain iris setpoint/readback and mount configuration. |
