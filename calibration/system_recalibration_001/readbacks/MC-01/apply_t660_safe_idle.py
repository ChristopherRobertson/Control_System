"""Apply and verify T660-only safe idle for MC-01; never connects to MIRcat."""
...
def main() -> int:
    status_path = HERE / "preconfiguration_safe_idle_status.json"
    record = {
        "campaign_id": "system_recalibration_001",
        "phase_id": "MC-01",
        "phase_run_id": "system_recalibration_001_MC-01_001",
        "started_utc": datetime.now(UTC).isoformat(),
        "purpose": "T660-only safe idle before GUI configuration transition",
    ...
    log = (HERE / "preconfiguration_safe_idle_command_log.txt").open(
        "a", encoding="utf-8"
    )
    ...
    try:
        readback = manager.apply_recipe(
            SAFE_IDLE,
            output_path=HERE / "preconfiguration_safe_idle_readback.json",
        )
