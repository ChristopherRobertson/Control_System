"""Issue one bounded MC-01 Process Trigger with the T660 engine started."""
...
def main() -> int:
    ...
    attempt = "retry1" if repeat == 1 else "attempt1"
    stem = f"repeat_{repeat}_{attempt}_started_process_trigger"
    record = {
        "authorization_id": "MC01-AUTH-002",
        "acquisition_id": f"MC01-ACTIVE-REPEAT-{repeat:03d}-{attempt.upper()}",
        "started_utc": datetime.now(UTC).isoformat(),
        "command_count_requested": 1,
        "engine_start_required": True,
        "channel": "T660-1 CHC",
    ...
    try:
        ...
        service.command("START", expect_response=False)
        ...
        record["status"] = "ONE_STARTED_TRIGGER_ISSUED"
    finally:
        try:
            service.command("STOP", expect_response=False)
            service.set_trigger_source("OFF")
