...
def test_compatibility_paths_keep_current_gui_assets_reachable():
    ...
    assert hardware_configuration_path() == (REPO_ROOT / "hardware_configuration.yaml").resolve()
    assert wiring_map_path() == (REPO_ROOT / "wiring_map.yaml").resolve()
    assert RECIPE_ROOT == REPO_ROOT / "recipes"
    assert RUN_ROOT == REPO_ROOT / "runs"
    assert LOG_ROOT == REPO_ROOT / "logs"
